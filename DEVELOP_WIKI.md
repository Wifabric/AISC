...
### 1.3 手测方案书写规范（2026-09-21 用户裁定）

交付自动化批次时写进 `docs/plans/<版本>-dev-plans/HANDTEST.md`（或对应计划文档）的每一条手测项，必须同时包含：

1. **操作**：编号步骤，精确到 UI 位置（哪个页签 / 哪个按钮 / 哪个菜单）或可直接复制执行的命令（PowerShell 形式；CLI 二进制与工作区路径用变量定义，如 `$ai`、`<WS>`，并在文首给示例值）。抽象描述（「三件套可用」「聚合正确」之类）不合格——那会把设计成本转嫁给验收人。
2. **理想结果（到字段级）**：JSON 输出要写明哪个数组几条、哪个字段非空 / 取什么值 / 什么顺序；UI 输出要写明看到什么文案 / 什么状态。用「正确 / 可用」这种结论词不算达标。
