...
| `aisc update` | `--check`, `--version`, `--from-file`, `--sha256`, `--rebuild` | CLI 自更新（冻结形态：rename dance 原子换 exe+相邻 bundle，旧件保留可回退；pip 形态给 pipx 指引；Workbench 池 mtime 驱逐免重启生效；`--pin-tool KEY=VALUE` 钉工具版本于数据根用户层、更新不丢；`--check` 附三工具版本对照） |
