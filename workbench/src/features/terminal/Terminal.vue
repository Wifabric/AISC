...
/** v2.1.13 (image-drag): OS images are accepted alongside the controlled
 *  workspace-path MIME. Windows WebView2 may report an empty File.type —
 *  the extension is the fallback signal. */
 ...
 *  quoted container path tokens into the PTY (no Enter, D11-09). */
async function uploadAndInsertImages(files: File[]): Promise<void> {
  const sid = sessionId.value;
  const ws = store.workspace;
  if (!sid || !sessionLive.value || !ws) {
    terminalNote(t("terminal.dropRejected"));
    return;
  }
  const tokens: string[] = [];
  let failed = 0;
  for (const f of files.slice(0, UPLOAD_MAX_FILES)) {
    try {
      ...
      failed += 1;
      terminalNote(
        t("terminal.dropImageFailed", { message: String(err) }),
        "error",
      );
    }
  }
  ...
  try {
    await writeSession(
      sid,
      Array.from(new TextEncoder().encode(tokens.join(" "))),
    );
    terminalNote(t("terminal.dropImageSaved", { count: tokens.length }));
    term?.focus();
  } catch {
    terminalNote(t("terminal.dropRejected"), "error");
  }
}
...
function onDragOver(e: DragEvent) {
  ...
  if (!types.includes(WORKSPACE_PATH_MIME) && !types.includes("Files")) return;
  e.preventDefault();
...
async function onDrop(e: DragEvent) {
  ...
  if (!relativePath || !sid || !sessionLive.value) {
    // OS image drag: the controlled MIME is absent by definition — fall
    // through to the image upload path when Files are present.
...
/** A-G03-2/A-G11-4: paste clipboard content into the session. v2.1.13: an
 *  IMAGE on the host clipboard uploads to .aisc/uploads and inserts its
 *  container path (container agents cannot read the host clipboard
 *  themselves); plain text falls back to the historical readText paste. */
async function doPaste() {
  try {
    ...
    if (rgba.length > 0) {
      ...
      try {
        const rel = await workspaceUploadImage(
          ws,
