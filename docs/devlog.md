...
- **发布**：cli-v0.1.2 tag 已打（run 防重入 + cache-inspect）；发布流水线
  首跑失败暴露 D-8.4 落地缺口——workflow_dispatch 执行 **main 分支**的工作流
  文件，而 cli-v final 判定修复只在 develop。已修三处（final 正则 /
  publish-pypi final-only gate / check-version-sync --tag）并合入 develop；
  main 快进合并按规约待用户执行后重发。
- **详细检测重设计（D-9b 手测 1-4）**：类别顺序钉死（镜像→容器→卷→构建
  缓存→网络，修复 HashMap 乱序漂移）；行改三列对齐（名称/大小/状态）；
  构建缓存按可回收降序、默认前 30 条 + 展开全部；docker 关闭点击 → 显式警告。
- **悬空标注**：改为「未被任何容器引用（dangling）」。
- **批 6 复测通过**：路径插入正确、toast 正常、20MiB 拦截 ✓。
- **待裁决**：同一 zip 恢复多个工作区——当前语义 = 按目标路径判重（同 zip
  恢复到不同路径合法，zip 是备份源）；若要「同一 zip 只恢复一次」需加
  manifest 身份登记，等用户裁定。
...
- r1（provider 模板化）裁定 **D-1/D-2/D-3**：**纯静态 AISC 自有模板**路线——维持现有
  「添加时选预设」形态与清单（5 家含 codesome），否决上游 `--config-file` 委托混合路、
  不镜像上游内置；cc-switch 8 个返佣赞助商模板**完全不出现**（无开关），
  **codesome 为 AISC 自有赞助商例外**；存量卷一次性迁移（marker bump 去第三方行、
  保留 official 与用户行）。去预配置改造时统一模板清单事实源——
  前后端漂移实证：`CcSwitchUiTab.vue:111` 硬编码 4 项 vs 容器预配置 5 项（UI 缺 codesome）。
- r2（codex computer use）裁定 **D-4**：**暂不原生实施**，闭环转观察（上游
  openai/codex#20851 契约稳定为重启先决；路线三按 §12.1 永久否决不变）；
  文档补「用户自挂 MCP 浏览器服务器」尝鲜路径。
- 本阶段新增 **codesome 专项（D-5）**：系统性修配置问题（UI 添加列表缺失、历史
  预配置配置项错误、切换超时之 codesome 适配、显示异常；完整清单待用户开题补充）。
- 裁决日志：`docs/plans/2.1.12-dev-plans/decisions.md`（D-1~D-5）。
