...
| A 链（P0） | **A1 元数据+改名 ✅（2026-09-16，CLI-A01 PASS：wheel/sdist/pipx 三腿 + build 零告警 + pytest 1238）** + D-4 版本切 0.x ✅；余 pip 五批次（指南 3.1-3.5）+ `aisc bundle fetch` 底座 + `aisc update`（sidecar 热换编排）+ 工具更新合一入口 + Workbench 自更新 + 0.1.0 final/PyPI 收口（Release 全量资产 + PyPI 首发） | 待做 | [a-pypi-and-update.md](a-pypi-and-update.md) |
