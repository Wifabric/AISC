...
- ~~U-3~~ → 已裁定（D-12：悬账全部收编）。
- ~~U-5~~ → **无需用户动作**（澄清：原意是请你回忆报障现场——当时新建
  provider 走的是模板还是自定义、claude 还是 codex 侧、保存后差异具体在
  哪个字段、拉取模型失败时选的哪家模板。这只是提高复现效率的回访，不是
  决策项：修复方案已覆盖全部假设（批 2 三假设 + 旧镜像 adapter 判别第 0
  步），日后若想起细节随时补充即可）。
...
- **「工作记录」六组歧义**（worklog-questionnaire.md）：待用户作答——
  最优先仅一问：一条工作记录 = 一个 bash 会话（A1）还是一个任务可挂多个
  会话（A2）？A2=批 3 原案复活规模 L，A1=收缩为账本可视化规模 M。
