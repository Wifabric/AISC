...
## 0. 设计铁律（D-15，2026-09-23 用户澄清）

**构建与宿主网络完全解耦**：不论启动摘要页选什么网络模式，都不应影响宿主
机网络环境，也**不得影响镜像构建**（网络模式仅作用于运行期容器 TUN 等）。
保底目标 = 无论宿主网络环境如何（断网 / GFW / TUN 代理 / 慢速），镜像构建
都有兜底措施保证成功——兜底 = **全预置离线路径**（本地命中零网络），
GH_PROXY 等显式通道仅是可选加速，不是兜底依赖。初步核实与现状一致：
`--network direct|proxy` 为 run 期旗标（main.py:167/657/675），build 链路
不消费；保底专项结论落地时按此铁律校验全链。
