...
模型重构（双安装器→修订为纯 Workbench 单安装器、preview 渠道、PyPI 解耦）、
版本双轨规约、阶段开发流程规约（§1.1）。全程见 devlog 与
archive/2.1.12-dev-plans/。

转期清单（→ v2.1.13 及后续）：
- [ ] 自更新端到端实测（preview.1 装机 → 检测升级下一版；随 v2.1.13 首个 preview 顺带）
- [ ] 旧卷迁移实测（等真实旧容器升级场景）
- [ ] r2 computer use 转观察（上游 openai/codex#20851 契约稳定再评估）
- [ ] Slurm/PBS 远程调用（阻塞：等用户提供实际工作流）
- [ ] provider 页「显示异常」（模板化后未再报告；如复现请附截图）
- [ ] 后备池不变：agent 加 Pi/opencode、docker 缓存自动清理、长对话无法恢复、历史页图标、飞书知识库、设备性能受限、网页版聊天界面/工作记录、变更页、导出 zip 恢复工作区
