...
# v2.1.13 开池基础（2026-09-19 整合：v2.1.12 转期 + 待解决全量并入，待开题点选）
...
**Provider / 配置页**
- [ ] provider 页「显示异常」（模板化后未再报告；复现请附截图）
- [ ] provider「编辑拆两个」（表头遗留）
- [ ] api-key 获取提示（表头遗留）
- [ ] agent 对话框补全/提示（插件）
...
**会话 / 工作区**
- [ ] 长对话无法恢复（bug）
- [ ] 历史会话重构：网页版聊天的界面 +「工作记录」概念——aisc 会话是一个 bash 包住 codex 会话的 runtime（记忆空间）；工作记录负责记录/保存打开过的 bash(bash 里 codex) 会话状态，与 codex 的 resume 记录非一一对应（惯用单 codex 会话=一一对应；一任务一干净会话=一对多）
...
**远程**
- [ ] 局域网通信增强 + Slurm/PBS 工作流针对性优化（阻塞：等用户提供实际工作流）

**文档**
- [ ] Claude/Codex 基本使用方法与技巧：可共同更新的协作文档（参考 Codesome 文档系统）
- [ ] 飞书知识库：同步在线文档，结合 Obsidian 管理本地知识库


# v2.1.12（已收官 2026-09-19 → 进入 v2.1.13；发布：v2.1.12-preview.1，final 经明确要求再发）

交付：provider 模板化 + codesome 专项（双模板/获取服务/行ID多实例/前缀校验）、
每模型思考等级多选 + 压缩阈值、编辑删除链路健壮化（DB-only 舞步）、D-8 发布
模型重构（纯 Workbench 单安装器、preview 渠道、PyPI 解耦）、版本双轨规约、
阶段开发流程规约（§1.1）。全程见 devlog 与 archive/2.1.12-dev-plans/。
转期与待解决已全量并入上方「v2.1.13 开池基础」。
