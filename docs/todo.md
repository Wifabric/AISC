...
- [ ] 历史会话重构·「工作记录」概念（2026-09-21 用户裁决移出当前版本，待捋清需求再立项）——工作记录负责记录/保存打开过的 bash(内含 codex) 会话状态，与 codex 的 resume 记录非一一对应（惯用单 codex 会话=一一对应；一任务一干净会话=一对多）；计划稿见 docs/plans/2.1.13-dev-plans/history-worklog.md
