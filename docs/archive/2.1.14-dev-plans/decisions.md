...
## 销账（2026-09-25，用户裁定维持 2.1.14 不开新版本）

> 用户原话口径：「这些功能好像都没什么问题啊，那就维持当前版本，把这些挂账都销掉吧」。

- **D-23（2026-09-25 用户追认，D-13 正式定稿）**：docker 管理破坏性操作边界按 D-13 草案定稿——清单制（五项之外的破坏性操作逐项须用户裁决）、owned/legacy_owned 之外只读不变、锁内重扫判权 + 前端 confirm + lease 三重不变量、永不触碰 AISC 外 docker 资产。实施已随 CLI 0.1.3 交付（`maintenance container-action`/`image-rm`/`image-tag`），用户日常使用确认无异常。
- **观察项销账**：T1-1/T1-2 应用内更新流 preview.2→3 实测 PASS（970f1b4）；T6-4/5/6 断网模拟、T5/T2-A/T4-1·3·4·5 日常使用回归——用户确认无异常，全部关闭。
- **U-4 终态**：推荐形态（操作菜单项 + 顶栏按钮 + 命令面板三入口）已交付且无异议，按交付形态定稿。
- **U-6 关闭**：对反馈 #1 匿名用户的追问无答复，随条目 resolved 一并关闭。
- **已知状态（非挂账）**：装机 sidecar CLI 停留 0.1.2——`aisc update` 网络下载已退役，无独立升级通道；随下一版 Workbench 安装器携带更新。
